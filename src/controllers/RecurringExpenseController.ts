...
export class RecurringExpenseController {
  async list(req: Request, res: Response) {
    try {
      ...
      if (startDate && endDate) {
        const list = await recurringExpenseService.listWithStatus(startDate as string, endDate as string);
        return res.status(200).json(list);
      }

      const recurringExpenses = await recurringExpenseService.listRecurringExpenses();
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      const recurringExpense = await recurringExpenseService.updateRecurringExpense(id, req.body);
  ...
  async launch(req: Request, res: Response) {
    try {
      ...
      const transaction = await recurringExpenseService.launch(id, user.id, new Date());
